...
def _build_self_contained_concat_graph(
...
def _build_self_contained_minimal_graph(*, input_has_audio: bool) -> str:
    if input_has_audio:
        return (
...
def is_trim_script_ready(
    *,
    ...
    script_path = get_trim_script_path(
        input_file=input_file,
        temp_dir=temp_dir,
        target_length=target_length,
        noise_threshold=noise_threshold,
...
def generate_trim_script(
    *,
    ...
    if len(plan.segments_to_keep) == 0:
        filter_graph = _build_self_contained_minimal_graph(input_has_audio=input_has_audio)
    else:
        filter_graph = _build_self_contained_concat_graph(
            plan.segments_to_keep,
            input_has_audio=input_has_audio,
        )

    write_filter_graph_script(script_path, filter_graph)
    return script_path
