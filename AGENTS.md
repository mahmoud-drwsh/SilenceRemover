...
- `src/ffmpeg/filter_graph.py`: Added `HEVC_QSV_FINAL_VIDEO_PAD` and `finalize_filter_graph_for_hevc_qsv()` to append NV12 + even-dimension scale after `[outv]` for Intel QSV after RGBA overlays.
- `src/ffmpeg/transcode.py`: `build_final_trim_command` takes `video_map_pad`; minimal overlay encode applies the same QSV tail and maps `outv_qsvsink` when codec is `hevc_qsv`.
- `src/media/trim.py`: Final trim filter script uses the QSV finalize path and matching `video_map_pad` when the resolved encoder is `hevc_qsv`.
