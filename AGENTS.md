...
- **Architecture and variants**: `src/app/pipeline.py` is the client-owned orchestrator; reusable media/FFmpeg/LLM modules live under `src/` and `packages/`; `remote-js/` is the Bun/Hono Media Manager. Each original has a stable source ID, with linked overlaid/no-overlay finals, designer uploads, originals, and subtitle actions on one canonical card; focused views collapse the hidden navigation rail and use the available grid width. Media lists page 24 cards at a time and prefetch the next page without changing unpaginated API callers.
...
- **Tag/state separation**: New videos use explicit variant, visibility, and publication fields; canonical-card companion lookups use that state with legacy-tag fallback. Only the video `trash` tag remains writable, while audio retains `todo`, `ready`, and `trash`. The fingerprint-guarded report-first rehearsal removes only obsolete video tags after proving view membership is preserved.
- **Public sharing retired**: Unauthenticated share-link creation, browsing, and streaming routes are removed; project-token designer/admin workflows are unchanged. Legacy `public_share_links` rows remain inert and startup neither creates nor checks that table.
