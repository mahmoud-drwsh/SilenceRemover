...
- **Progress output**: Every phase precomputes skip decisions before rendering, emits one aggregate `Skipped N/total` line, and iterates only runnable files; skipped inventories never cause per-file stdout writes or flushes.
- **Raw preflight output**: Completed-marker and locked-file decisions are classified before media probing and emitted as one aggregate summary; only candidates requiring duration/silence analysis enter the expensive loop.
- **Link maintenance output**: The short-video linker checks existing destinations before ffprobe, counts them silently, and reports one aggregate existing-link total instead of one line per link.
