...
- **Pipeline run**: Always executes phases 1–3 (transcription, title, final MP4); video encoder profile is resolved at startup via `resolve_video_encoder()` (QSV-primary; see **Encoding**). After each successful Phase 3 encode, optional Telegram text via `sr_telegram_notify.notify_final_output_ready` (see **Transcription… packages**). The former `--llm-only` mode, `temp/titles.txt` run log, and `pwsh/Start-VerticalLlmDryRun.ps1` were removed.
...
- `packages/sr_telegram_notify/`: New package with `notify_final_output_ready` and internal `sendMessage` client (`httpx`); reads optional Telegram env at call time; failures do not raise.
- `src/app/pipeline.py`: Phase runners receive `(video_path, video_index, total_videos)`; Phase 3 calls `notify_final_output_ready` after successful `trim_single_video`, before `mark_completed`.
