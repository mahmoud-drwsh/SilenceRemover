...
- **Pipeline run**: Always executes phases 1–3 (transcription, title, final MP4); video encoder profile is resolved at startup via `resolve_video_encoder()` (QSV-primary; see **Encoding**). The former `--llm-only` mode, `temp/titles.txt` run log, and `pwsh/Start-VerticalLlmDryRun.ps1` were removed.
