...
- **Pipeline run**: Always executes phases 1–3 (transcription, title, final MP4); `libx265` encoder profile is resolved at startup. The former `--llm-only` mode, `temp/titles.txt` run log, and `pwsh/Start-VerticalLlmDryRun.ps1` were removed.
