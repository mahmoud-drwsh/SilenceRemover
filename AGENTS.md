...
- **No-overlay completion**: Companion MP4s are stored under `temp/no_overlay/`; `temp/no_overlay_completed/{source-id}.txt` is independent from the standard completion marker. It prevents regeneration after the companion MP4 has been moved and can adopt existing local/server companions.
