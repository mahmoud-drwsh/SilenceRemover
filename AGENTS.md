...
- **Tag/state separation**: New videos use explicit variant, visibility, and publication fields; only the video `trash` tag remains writable, while audio retains `todo`, `ready`, and `trash`. Legacy reads remain compatible, and the fingerprint-guarded report-first rehearsal removes only obsolete video tags after proving view membership is preserved.
