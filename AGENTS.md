...
- **Upload eligibility**: Only `Start-VerticalVideoProcessing.ps1` inherits `MEDIA_MANAGER_URL` and uploads Originals for server processing. The horizontal launcher explicitly sets it empty for its child process (so `.env` cannot restore it), keeping horizontal media local.
