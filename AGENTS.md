...
- **Video variants**: Each source can have an overlaid video and a silence-removed no-overlay companion. No-overlay companions use the `no-overlay` tag/folder and share their original’s source ID. Startup surgically backfills only empty-tag legacy companions whose `-no-overlay` ID exactly matches a linked original source ID.
