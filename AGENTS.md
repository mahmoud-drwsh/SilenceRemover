...
- **Upload eligibility**: Only `Start-VerticalVideoProcessing.ps1` uploads Originals for server processing. The horizontal launcher keeps video bytes and its one silence-removed output local; it uses Media Manager only for the transient transcript/title request—no subtitles, overlays, companion output, or media upload.
- **Transient snippet analysis**: Authenticated clients may submit one bounded local OGG snippet for server-side OpenRouter transcription and one-line title generation. The Media Manager neither stores the snippet nor creates a media/metadata record; provider URL, key, and models are service configuration.
- **Video lists**: Every video view returns one canonical pipeline-final card per original. The original, no-overlay, designer, and subtitle variants are linked actions on that card; Needs Designer remains focused to cards without a designer upload.
