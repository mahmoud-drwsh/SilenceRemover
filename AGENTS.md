...
- `src/ffmpeg/runner.py`: `format_ffmpeg_process_failure` and threaded stderr drain in `run_with_progress` (unchanged this session).
- `src/ffmpeg/silence_removed_runner.py`: Failed FFmpeg runs raise `RuntimeError` with an stderr excerpt (unchanged this session).
