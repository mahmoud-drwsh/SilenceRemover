...
- **Original-root safety audit**: The authenticated `original-rooted-rehearsal` route is report-only: it produces the guarded backfill fingerprint from the private service database and has no HTTP apply path.
