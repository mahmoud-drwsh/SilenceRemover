...
- **Variants, storage, and overlays**: New derived uploads require an existing original `source_id`; explicit variant/review/visibility/publication fields drive virtual views with legacy fallback. Designer uploads are immutable revisions whose newest successful upload becomes active. Admins manage one checksum-tracked PNG logo per project for overlaid rendering only; no-overlay finals stay logo-free.
