...
- **Upload eligibility**: Only `Start-VerticalVideoProcessing.ps1` inherits `MEDIA_MANAGER_URL` and uploads Originals for server processing. The horizontal launcher removes that variable for its child process, so horizontal media is never uploaded to Media Manager.
