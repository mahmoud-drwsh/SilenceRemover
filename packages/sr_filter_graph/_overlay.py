...
def _overlay_suffix_after_concat(
    *,
    ...
    Args:
        title_overlay_y: Y position for title banner overlay (None = no title)
    ...
    print(f"[DEBUG FILTER] _overlay_suffix_after_concat: has_title={has_title}, has_logo={has_logo}, logo_enabled={logo_enabled}")
    if not has_title and not has_logo:
        print("[DEBUG FILTER] Both title and logo disabled - returning empty suffix")
        return ""
    parts: list[str] = []
    base_label = "outv"
    logo_stream_idx = 2 if has_title else 1
    print(f"[DEBUG FILTER] logo_stream_idx={logo_stream_idx} (has_title={has_title})")
    if has_logo:
        ...
        print(f"[DEBUG FILTER] Added logo filter: [{logo_stream_idx}:v]... -> [{logo_out}]")
        base_label = logo_out
    if has_title:
        oy = int(title_overlay_y)  # type: ignore[arg-type]
        title_out = "outv_title"
        title_filter = f"[1:v]format=rgba[ov_title];[{base_label}][ov_title]overlay=0:{oy}:shortest=1[{title_out}]"
        parts.append(title_filter)
        print(f"[DEBUG FILTER] Added title filter: [1:v]... -> [{title_out}] at y={oy}")
    ...
    result = ";" + ";".join(parts)
    print(f"[DEBUG FILTER] Final suffix length: {len(result)} chars")
    return result
