"""Tests for sr_mp3_manager package."""
...
class TestMp3ApiClient:
    """Test HTTP client initialization and methods."""
    
    def test_init_from_url(self):
        """Parse full URL correctly."""
        client = Mp3ApiClient("https://example.com/TOKEN123/arabic-lessons/")
        assert client.base_url == "https://example.com"
        assert client.token == "TOKEN123"
        assert client.project == "arabic-lessons"
    
    def test_init_from_env(self, monkeypatch):
        """Read from MP3_MANAGER_URL env var."""
        monkeypatch.setenv("MP3_MANAGER_URL", "https://host.com/tok/proj/")
        client = Mp3ApiClient()
    ...
    def test_init_missing_url(self):
        ...
        with pytest.raises(ValueError, match="MP3_MANAGER_URL"):
            Mp3ApiClient()
...
class TestSyncTitles:
    ...
    def test_sync_updates_different_titles(self, tmp_path):
        ...
        (completed_dir / "video.mp4.json").write_text("{}")
        ...
        assert "video.mp4" in updated
        assert (titles_dir / "video.mp4.txt").read_text() == "New Title"
        assert not (completed_dir / "video.mp4.json").exists()
    ...
    def test_sync_skips_same_titles(self, tmp_path):
        ...
        assert "video.mp4" not in updated
        assert (completed_dir / "video.mp4.json").exists()  # Not deleted
    ...
    def test_sync_ignores_api_missing(self, tmp_path):
        ...
        assert (completed_dir / "video.mp4.json").exists()  # Preserved
    
    def test_sync_api_failure_safe(self, tmp_path):
        """When API fails, return empty list (fail-safe)."""
        client = Mock()
        client.get_all_files.side_effect = Exception("Network error")
        
        updated = sync_titles(client, tmp_path / "titles", tmp_path / "completed")
        
        assert updated == []  # Safe fallback


class TestEnsureUploaded:
    ...
    def test_upload_if_not_exists(self, tmp_path):
        ...
        result = ensure_uploaded(client, "video.mp4", "Title", audio_file)
